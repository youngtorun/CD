...
public class AdminAddCdServlet extends HttpServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		sfu.setFileSizeMax(3000 * 1024);//设置单个上传的文件上限为80KB
		/*
		...
		/*
		 * 4. 把List<FileItem>封装到Cd对象中
		 * 4.1 首先把“普通表单字段”放到一个Map中，再把Map转换成Cd和Category对象，再建立两者的关系
		 */
		...
		Cd cd = CommonUtils.toBean(map, Cd.class);//把Map中大部分数据封装到Cd对象中
		 ...
		 * 1. 获取真实路径
		 */
		String savepath = this.getServletContext().getRealPath("/cd_img");
		/*
		 * 2. 创建目标文件
		 */
		...
		// 3. 获取宽高来进行校验
		if(image.getWidth(null) > 3000 || image.getHeight(null) > 3000) {
		 ...
		 * 1. 获取真实路径
		 */
		savepath = this.getServletContext().getRealPath("/cd_img");
		/*
		 * 2. 创建目标文件
		 */
		...
		// 把图片的路径设置给cd对象
		cd.setImage_b("cd_img/" + filename);
		
		
		
		
		// 调用service完成保存
		cd.setBid(CommonUtils.uuid());
		CdService cdService = new CdService();
		cdService.add(cd);
		
		// 保存成功信息转发到msg.jsp
		request.setAttribute("msg", "添加唱片成功！");
	...
	private void error(String msg, HttpServletRequest request, HttpServletResponse response)
		...
		request.getRequestDispatcher("/adminjsps/admin/cd/add.jsp").
